...
public class ShortestPaths {
    ...
    public void run() {
    	...
    	//
    	while(!pq.isEmpty()) {
    		VertexAndDist ud = pq.extractMin();
    		int minDistance = ud.distance;
    		Vertex u = ud.vertex;
    		
    		for(Edge e: u.edgesFrom()) {
    			Decreaser<VertexAndDist> distanceOfEdges = handles.get(e.to);
    			int newDistance = minDistance + weights.get(e);
    			
    			if(newDistance < distanceOfEdges.getValue().distance) {
    				Vertex v = distanceOfEdges.getValue().vertex;
    				VertexAndDist updateDistance = new VertexAndDist(v, newDistance);
    				distanceOfEdges.decrease(updateDistance);
